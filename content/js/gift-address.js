(function(){
	...
	giftForm.find('input[name="giftId"]').val(id);
	giftForm.find('input[name="memberId"]').val(uid);
	giftForm.find('input[name="openId"]').val(config.getOpenId());
	giftForm.find('input[name="model"]').val(
			location.search.getQueryValue('model'));
	giftForm.find('input[name="color"]').val(
			location.search.getQueryValue('color'));
	...
	function saveGift(){
		Ajax.submitForm({
			url : config.giftApply,
			data : giftForm
		}, function(data) {
			if(data.code != 'OK'){
				if(data.message == '您不是首次关注'){
